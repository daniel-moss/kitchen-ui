...
import AvatarInvoice from "./AvatarInvoice";
import { AvatarInvoiceStatus } from "./AvatarInvoice.types";
import { AvatarSize } from "./Avatar.types";

const SIZES: AvatarSize[] = ["xxs", "xs", "sm", "md", "lg", "xl"];
const STATUSES: AvatarInvoiceStatus[] = [
  "none",
  "draft",
  "unsent",
  "outstanding",
...
/**
 * AvatarInvoice — an Avatar template: object/icon avatar with the `invoice`
 * semantic icon, and a `status` shown as the corner icon addOn. xxs supports
 * `none` only (no addOns on xxs).
 */
const meta: Meta<typeof AvatarInvoice> = {
  title: "Components/Avatar/AvatarInvoice",
  component: AvatarInvoice,
...
type Story = StoryObj<typeof AvatarInvoice>;
          ...
          {SIZES.map((size) => (
            <div key={size}>
              ...
              {size === "xxs" && status !== "none" ? null : (
                <AvatarInvoice size={size} status={status} />
              )}
            </div>
          ))}
        </Fragment>
