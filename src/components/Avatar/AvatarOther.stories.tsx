...
import AvatarOther from "./AvatarOther";
import { AvatarOtherStatus } from "./AvatarOther.types";
import { AvatarSize } from "./Avatar.types";

const SIZES: AvatarSize[] = ["xxs", "xs", "sm", "md", "lg", "xl"];
const STATUSES: AvatarOtherStatus[] = ["none", "active", "review", "inactive"];
...
type Story = StoryObj<typeof AvatarOther>;
    ...
    >
      ...
      {STATUSES.map((status) => (
        <Fragment key={status}>
          ...
          {SIZES.map((size) => (
            <div key={size}>
              {size === "xxs" && status !== "none" ? null : <AvatarOther size={size} status={status} />}
            </div>
          ))}
        </Fragment>
      ))}
