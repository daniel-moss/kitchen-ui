...
import styles from "./SidebarNavWorkspaceButton.module.scss";
import { SidebarNavWorkspaceButtonProps, WorkspaceItem } from "./SidebarNavWorkspaceButton.types";
...
function WorkspaceAvatar({ workspace, size }: { workspace: WorkspaceItem; size: "md" | "xs" }) {
  return (
    <Avatar
      ...
      characters={workspace.name.charAt(0)}
    />
  );
}

// SidebarNavWorkspaceButton — switches between workspaces: a compact button
...
// Figma "SidebarNavWorkspaceButton".
export default function SidebarNavWorkspaceButton({
  workspaces,
  value,
  defaultValue,
  onChange,
  ...
  breakpoint,
  className,
}: SidebarNavWorkspaceButtonProps) {
