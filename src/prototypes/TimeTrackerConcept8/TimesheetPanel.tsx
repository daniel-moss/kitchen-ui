...
// Check-in status → its icon (Figma: van = Travelling, wrench = Working).
export const categoryIcon = (category?: string) =>
  category === "Travelling" ? "van" : category === "Working" ? "wrench-simple" : "stopwatch";
...
const ActiveSessionRow = ({
  ...
  const statusItem = (status: string) => (
    <MenuItem
      ...
      slotRight={status === session.category ? <Icon icon="check-circle" pack="solid" size={16} container="square" className={styles.statusCheck} /> : undefined}
      onClick={() => {
        menu.close();
...
interface TimesheetPanelProps {
   ...
   *  Receives the group's user id — with `viewerId` unset (office) every
   *  group has a plus, and the new session must land in THAT group. */
