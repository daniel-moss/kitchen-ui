...
import TimesheetPanel, { categoryIcon, formatHrMin, Session } from "./TimesheetPanel";
...
export interface JobDetailsProps {
  ...
  /**
   * OFFICE view (the "Office / Desktop" story). The office manages the job but
   * does NOT track time: no Check in card in Start/Resume, no session bar/pill,
   * no pause banner, Summary tab is a placeholder — but the Timesheet lets the
   * office add/edit sessions for EVERY assignee.
   */
  office?: boolean;
}
    ...
    <MenuItem
      ...
      subMenu={
        <MenuItemGroup>
          ...
          <MenuItem label="Job series" slotLeft={slot(semanticIcons.series)} />
        </MenuItemGroup>
      }
...
interface StoredSession {
  id: number;
  /** The assignee the session belongs to. Unset = the viewer (the office view
   *  adds sessions for other assignees too). */
  userId?: number;
  startLabel: string;
  month: string;
  day: string;
...
const SessionBar = ({
  ...
  checkedIn: boolean;
  elapsed: number;
  /** The running session's check-in status ("Travelling" / "Working"). */
  category?: string;
  onCheckIn: () => void;
  onCheckOut: () => void;
  onSwitchStatus: (status: string) => void;
}) => {
  // Active bar's ellipsis → the Time Session Context Menu (Figma 24192-73141).
  const menu = useAnchoredMenu(true, "end");
  const statusItem = (status: string) => (
    <MenuItem
      label={status}
      slotLeft={slot(categoryIcon(status))}
      slotRight={status === category ? <Icon icon="check-circle" pack="solid" size={16} container="square" className={styles.statusCheck} /> : undefined}
      onClick={() => {
        menu.close();
      ...
      <div className={styles.timerRow}>
        <div className={styles.timerInfo}>
          <span className={clsx(styles.timerAvatar, !checkedIn && styles.timerAvatarIdle)}>
            <Icon icon={checkedIn ? categoryIcon(category) : "warning"} pack="solid" size={16} />
          </span>
          <span className={styles.timerText}>
            ...
            <span className={clsx(styles.timerLabel, !checkedIn && styles.timerLabelIdle)}>
              {checkedIn ? category ?? "Tracking your time" : "Not tracking your time"}
            </span>
          </span>
        </div>
        {checkedIn ? (
          ...
          <Menu open={menu.open} onClose={menu.close} breakpoint="desktop">
            <MenuItemGroup>
              <MenuItem
                ...
                tag={category}
                subMenu={<MenuItemGroup>{[statusItem("Travelling"), statusItem("Working")]}</MenuItemGroup>}
              />
            </MenuItemGroup>
            <MenuItemGroup>
              <MenuItem
...
const SessionPill = ({
  ...
  <button
    ...
    onClick={onClick}
  >
    <Icon icon={checkedIn ? categoryIcon(category) : "warning"} pack="solid" />
...
// The mobile "Time tracker" dialog (Figma 24178-58902 — now a Dialog) —
// opened by tapping the active pill. Title header + close, body = the current
// STATUS caption + big red timer + a labeled Travelling/Working radio pair
// (a draft pick); footer = Cancel, ghost "Check out" (→ the prompt) and solid
// "Update", which commits a status switch. Dismissing (scrim / X) with an
...
const SessionDrawer = ({
  ...
  // The radios are a DRAFT — only Update commits (Figma annotation).
  ...
  return (
    ...
    >
      <div className={styles.sessionDrawerBody}>
        ...
        <Input label="What time do you want to track?">
          <RadioGroup orientation="horizontal" value={draft} onChange={setDraft}>
            <RadioItem value="Travelling" variant="card" icon="van" iconPack="regular" label="Travelling" />
            <RadioItem value="Working" variant="card" icon="wrench-simple" iconPack="regular" label="Working" />
          </RadioGroup>
        </Input>
      </div>
    </Dialog>
...
// opened from the idle bar's "Check in" button and the idle pill. Pick a
// status and check in; confirming starts the session ("You're checked in").
const CheckInDialog = ({
  open,
  onClose,
  onCheckIn,
...
function useJobShell(isDesktop: boolean) {
  ...
  // Per-assignee sessions — a stored session belongs to its `userId` (unset =
  // the viewer; the office adds sessions for other assignees too).
  const sessionsByUser: Record<number, Session[]> = {};
  displaySessions.forEach((ds, i) => {
    const uid = sessions[i].userId ?? viewer.id;
    (sessionsByUser[uid] ??= []).push(ds);
  });
  ...
  // Prompt confirmed (Figma 24113-16303): the session is logged directly —
  // no Time review in this concept. The bar/pill fall back to the amber
  // "Not tracking your time" state (job still active, no session running).
  ...
  // `userId` = which assignee's group the plus was clicked in (office view);
  // the guard drops non-number values (e.g. a click event from a direct call).
  const [addTarget, setAddTarget] = useState<number | undefined>();
  const openAddSession = (userId?: number) => {
    setAddTarget(typeof userId === "number" ? userId : undefined);
  ...
  const saveSession = (draft: SessionDraft) => {
    ...
    setSessions((prev) =>
      editingSession != null
        ...
        : [...prev, { id: prev.reduce((mx, s) => Math.max(mx, s.id), 0) + 1, userId: addTarget, ...fields }],
    );
  ...
  const actions: JobActions = {
    ...
    // Complete job has NO dialog for now (Daniel, 2026-07-22) — the button and
    // menu item stay, but clicking only closes the menu.
    onComplete: () => {
      actionMenu.close();
    },
    onCheckIn: checkIn,
...
const JobForms = ({ s, mobile = false, office = false }: { s: ShellState; mobile?: boolean; office?: boolean }) => (
  <>
    <PauseJobForm open={s.pauseOpen} onClose={() => s.setPauseOpen(false)} onPause={s.pauseJob} mobile={mobile} showBanner={!office} />
...
const DesktopShell = ({ office = false }: { office?: boolean }) => {
  ...
  return (
    <div className={styles.desktop}>
      ...
      <div className={styles.workArea}>
        ...
        <div className={styles.contentRow}>
          <ScrollArea wrapperClassName={styles.mainArea} className={styles.mainAreaScroll}>
            ...
            {tab === "service" ? (
              <div className={styles.mainContent}>
                <ServicePanel serviceValues={s.serviceValues} onServiceChange={s.setServiceValues} serviceLocked={!office} />
              </div>
            ) : tab === "timesheet" ? (
              <div className={styles.mainContent}>
                <TimesheetPanel
                  assignees={s.assigneeUsers}
                  // Office: every assignee's group is editable + gets the plus.
                  viewerId={office ? undefined : s.viewer.id}
              ...
              // Summary + Activity are placeholders in this concept — every
              // story, tech and office alike (Daniel, 2026-07-22).
          ...
          <ScrollArea wrapperClassName={styles.sidebar} className={styles.sidebarScroll}>
            ...
            {!s.locked && (
              <div className={styles.sidebarHeader}>
                ...
                {/* The session bar shows on ACTIVE status only (Daniel's rule) —
                    in both its states: running session or "Check in". The
                    OFFICE has no time tracking, so no bar at all. */}
                {s.job.status === "active" && !office && (
                  <>
                    <SessionBar
            ...
            {/* Techs cannot edit the Details modules (Daniel, 2026-07-22):
                locked hides every pencil/Add and trims the contact menus to
                the copy actions. Only the OFFICE edits. */}
            <DetailsPanel scheduling={s.scheduling} onSchedulingChange={s.setScheduling} job={s.job} locked={s.locked || !office} recallTo={s.serviceValues.type === "recall" ? s.serviceValues.recallTo : null} />
          </ScrollArea>
        </div>
      </div>
      ...
      <StartJobForm open={s.startOpen} onClose={() => s.setStartOpen(false)} onStart={s.startJob} showCheckIn={!office} />
      <CancelJobForm open={s.cancelOpen} onClose={() => s.setCancelOpen(false)} scheduling={s.scheduling} onCancel={s.cancelJob} />
      <JobForms s={s} office={office} />
...
const MobileShell = ({ office = false }: { office?: boolean }) => {
    ...
    <div className={styles.mobile}>
      <ScrollArea wrapperClassName={styles.mobileScroll} className={styles.mobileScrollInner}>
        ...
        <SwipePager
          ...
          <DetailsPanel mobile scheduling={s.scheduling} onSchedulingChange={s.setScheduling} job={s.job} locked={s.locked || !office} recallTo={s.serviceValues.type === "recall" ? s.serviceValues.recallTo : null} />
            ) : t === "service" ? (
              <ServicePanel mobile serviceValues={s.serviceValues} onServiceChange={s.setServiceValues} serviceLocked={!office} />
            ) : t === "timesheet" ? (
          <TimesheetPanel
            assignees={s.assigneeUsers}
            viewerId={office ? undefined : s.viewer.id}
      ...
      <StartJobForm open={s.startOpen} onClose={() => s.setStartOpen(false)} onStart={s.startJob} showCheckIn={!office} mobile />
      <CancelJobForm open={s.cancelOpen} onClose={() => s.setCancelOpen(false)} scheduling={s.scheduling} onCancel={s.cancelJob} mobile />
      <JobForms s={s} mobile office={office} />
      <Toaster breakpoint="mobile" />
    </div>
  );
};

export default function JobDetails({ breakpoint = "auto", office = false }: JobDetailsProps) {
  const isDesktop = useIsDesktop(breakpoint);
  return isDesktop ? <DesktopShell office={office} /> : <MobileShell office={office} />;
}
