...
// module (keep in sync). "Inherit from location" and "Bill to location" both
 ...
 *  - inherit / location → the service LOCATION item
 *  - client            → the service CLIENT item
 *  - differentClient   → the chosen client item
 */
export function billingSummary(b: Billing): { kind: "location" | "client"; name: string; caption: string } {
  ...
  return { kind: "location", ...SERVICE_LOCATION };
}
