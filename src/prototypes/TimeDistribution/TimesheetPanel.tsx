...
export interface Session {
  ...
  timerLabel: string;
  /**
   * Session category from the check-out Time review split ("Travelling" /
   * "Working"). When set, the row caption shows it instead of the date.
   */
  category?: string;
}
...
export const ceil15Min = (min: number) => Math.ceil(min / QUARTER_MIN) * QUARTER_MIN;
...
const ActiveSessionRow = ({ session, onStop }: { session: Session; onStop: () => void }) => (
  <ListItem
    ...
    avatar={<ActiveDot />}
    slotRight={
      <Button size="lg" variant="ghost" leftIcon="arrow-left-from-arc" onClick={onStop}>
        Check out
      </Button>
    }
  />
...
const EndedSessionRow = ({ session, mobile, editable, onEdit, onDelete }: { session: Session; mobile: boolean; editable: boolean; onEdit: () => void; onDelete: () => void }) => {
  ...
  const caption = session.category ?? (crossDay ? `${session.dateLabel} → ${session.endDateLabel}` : session.dateLabel);
  const menuBody = (
    <>
      <MenuItemGroup>
        <MenuItem
